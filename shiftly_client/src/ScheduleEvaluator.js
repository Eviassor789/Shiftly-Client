class ScheduleEvaluator {
  ...
  initialize() {
    ...
    Object.entries(this.relevantShifts).forEach(([shiftId, shiftInfo]) => {
      this.shiftsToRequirementsMap[shiftId] =
        this.getRelevantRequirementsForShift(shiftInfo);
    });
  ...
  getFitnessWithMoreInfo(solution) {
    ...
    solution.forEach(([workerId, shiftId]) => {
      ...
      cost += this.relevantShifts[shiftId]
        ? this.relevantShifts[shiftId].cost
        : 0;

      // Update constructs
      constructs[workerId] -= this.durationOfShift(shiftId);

      // Update leftRequirements
      if (this.getRelevantRequirementsForShift(this.relevantShifts[shiftId])) {
        this.getRelevantRequirementsForShift(
          this.relevantShifts[shiftId]
        ).forEach((req) => {
          // const requirementId = requirement.id;
          if (leftRequirementsNumMap[req.day + req.profession + req.hour] > 0) {
            leftRequirementsNumMap[req.day + req.profession + req.hour] -= 1;
          }
        });
      }
    });
    ...
    totalRequirementsNum = this.requirements.reduce(
      (sum, req) => sum + req.number,
      0
    );

    // Calculate satisfied requirements
    satisfiedRequirements =
      totalRequirementsNum -
      Object.values(leftRequirementsNumMap).reduce((sum, num) => sum + num, 0);
    ...
    let maxRequirementsMap = { ...this.maxRequirementsMap };
